...
<CustomCodeBlock language={"sql"}>
  -- Users get -- All activities from users they follow -- All join activities
  -- All activities of anybody following them CREATE OR REPLACE FUNCTION
  social_feed(for_user_id uuid) RETURNS SETOF activities LANGUAGE sql AS $$
  SELECT DISTINCT ON (feed.id) feed.* FROM ( SELECT activities.* FROM activities
  JOIN follows ON activities.user_id = follows.followee_id WHERE
