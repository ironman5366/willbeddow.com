...
## What I didn't like: Deno and Tamagui

The other new technologies I used in this project were Deno, for the Supabase edge functions, and [Tamagui](https://tamagui.dev/),
a cross-platform React UI library. I actually have the same complaint about both, even though they're extremely
different technologies: in doing the cool things they do, I feel that they go over the complexity cliff and make
developer experience worse.

In Deno's case, the new URL based imports don't yet integrate well with any of the IDEs I use, and I find it difficult to debug.
You could argue that this will change as adoption increases, but today my experience is that it's a pain, and
makes development slower. I also just in general don't love the URL imports - I understand the arguments
about mitigating supply chain issues, pinning dependencies more explicitly, and acting more like a one-off script, but it's not my preference.

Tamagui is a totally different beast. Deno is a mature piece of engineering that I happen to not jive with yet, but
Tamagui feels, while impressive, immature. There's an extremely complex system of theme overrides, tokens, and
custom configuration that tries to take over the app - I found myself fighting with it constantly. For instance, every
Tamagui component accepts every possible style prop outside of the `style` key. So this is a valid tamagui component:
<CodeBlock language="tsx">
```tsx
<Button
  // the normal style prop
...
Components are compiled at build-time, and I ran into bugs several times with component compilation.
