...
export default function BlogTable() {
    ...
    return <Table c={"wine"}>
        ...
        <Table.Tbody>
            {posts && posts.postConnection && posts.postConnection.edges && posts.postConnection.edges.map((post) =>
                <Table.Tr key={post?.node?.id} style={{
                    cursor: "pointer",
                    ...
                    <Table.Td>
                        <Link href={getDocumentPath(post?.node)} key={post?.node?.id}>
                            {post?.node?.created_at && <FormattedDate isoString={post.node.created_at} />}
                        </Link>
                    </Table.Td>
                </Table.Tr>
            )}
